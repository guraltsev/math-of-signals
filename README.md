This is a collection of JupyterLab notebooks for exploring Fourier analysis


# Activities

- [Compare sine, triangle, square, and composite waveforms while connecting their shapes to the sounds they produce.](https://guraltsev.github.io/math-of-signals/lab/index.html?path=How_functions_sound.ipynb&mode=single-document)
- [Match a mystery function with a trig expansion by adjusting the sine coefficients.](https://guraltsev.github.io/math-of-signals/lab/index.html?path=Is_F_sum_of_trig.ipynb&mode=single-document)
- [Approximate a square wave with a finite trig sum.](https://guraltsev.github.io/math-of-signals/lab/index.html?path=Is_Square_sum_of_trig.ipynb&mode=single-document)
- [Explore products of sine waves and see how different frequencies interact on the unit interval.
](https://guraltsev.github.io/math-of-signals/lab/index.html?path=How_to_find_those_trig_functions.ipynb&mode=single-document)
- [Review the relations that make Fourier expansions work.
](https://guraltsev.github.io/math-of-signals/lab/index.html?path=The_Fourier_coefficients.ipynb&mode=single-document)

# Sounds


The UArk Fight song
- Format: MP3
- Encoding: 44.1 kHz stereo, 192 kb/s
- Duration: 00:01:24.27
- File size: 2,023,268 bytes
[Uark-fight.mp3](https://guraltsev.github.io/math-of-signals/lab/index.html?path=Uark-fight.mp3&mode=single-document)

The UArk Fight song (very compressed)
- Format: MP3
- Encoding: 44.1 kHz stereo, 32 kb/s
- Duration: 00:01:24.27
- File size: 337,430 bytes
[Uark-fight-32.mp3](https://guraltsev.github.io/math-of-signals/lab/index.html?path=Uark-fight-32.mp3&mode=single-document)

The UArk Fight song (downsampled)
- Format: WAV
- Encoding: 8 kHz mono PCM, 128 kb/s
- Duration: 00:01:24.27
- File size: 1,347,950 bytes
[uark-fight-low-sample.wav](https://guraltsev.github.io/math-of-signals/lab/index.html?path=uark-fight-low-sample.wav&mode=single-document)

# Visual compression

 [How to use ideas from sound encoding for image encoding - Basics of JPEG
](https://guraltsev.github.io/math-of-signals/lab/index.html?path=Image_Fourier_Approximations.ipynb&mode=single-document)